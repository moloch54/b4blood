...
printf(' scanning for anonymous shares',green)
os.system(f"smbmap -H {ip_to_scan} -u '' -p '' -d {Domain_Name}")
...
if len(contenu) !=0:
	...
	if smb==1:
		...
		os.system(f"nmap {ip_to_scan}  -Pn --script vuln -p 445 -v | grep 'VULNERABLE:' -C 1")
	...
	for cred in contenu:
		...
		if smb:
			...
			os.chdir("smb_dump")
			file_content="admin,account,compte,cred,user,passw,note,vbs,ps1,bat,exe,conf,cfg,rsa,pem,key,xml,xslx,doc,docx,zip,txt,pdf,pfx".split(",")
			for share in shares:
				share=share.replace("\n","")
				share=share.replace(" ","")
